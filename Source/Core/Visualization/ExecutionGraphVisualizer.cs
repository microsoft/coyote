...
namespace Microsoft.Coyote.Visualization
{
    ...
    /// </summary>
    internal sealed class ExecutionGraphVisualizer
    {
        /// <summary>
        /// Visualizes the specified <see cref="ExecutionGraph"/> and returns it in DOT format.
        /// </summary>
        internal static string Visualize(ExecutionGraph graph, Layout layout) => ToGraphViz(graph, layout);

        /// <summary>
        /// Visualizes the specified <see cref="ExecutionGraph"/> and returns it in DOT format.
        /// </summary>
        private static string ToGraphViz(ExecutionGraph graph, Layout layout)
        {
            ...
            {
                ClusterCallSitesPerOperation(sb, graph, edgeCache);
            }
            else
            {
                ClusterCallSitesPerType(sb, graph);
            }
            ...
            foreach (var node in graph)
            {
                ...
                {
                    if (TryCreateEdgeStatement(edge, layout, edgeCache, out string edgeStmt))
                    {
        ...
        /// </summary>
        private static void ClusterCallSitesPerOperation(StringBuilder sb, ExecutionGraph graph, HashSet<string> edgeCache)
        {
            ...
            foreach (ulong operation in operations)
            {
                ...
                sb.AppendLine($"    label=\"Operation #{operation}\";");
                ...
                foreach (var node in nodes)
                {
                    ...
                    {
                        if (TryCreateEdgeStatement(edge, Layout.Trace, edgeCache, out string edgeStmt))
                        {
                            sb.AppendLine($"    {edgeStmt};");
                        }
                    }
        ...
        /// </summary>
        private static void ClusterCallSitesPerType(StringBuilder sb, ExecutionGraph graph)
        {
        ...
        /// </summary>
        private static bool TryCreateEdgeStatement(Edge edge, Layout layout, HashSet<string> cache, out string result)
        {
            ...
            result = $"{edgeStmt} [{string.Join(",", attributes)}]";
            return true;
        }
