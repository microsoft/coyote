    ...
    public class SendAndExecuteTests : BaseSystematicTest
    {
        ...
        private class M1A : StateMachine
        {
            ...
            private async Task InitOnEntry(Event e)
            {
                ...
                {
                    b = await this.Manager.CreateActorAndExecuteAsync(typeof(M1B));
                }
                else
                {
                    b = this.Manager.CreateActor(typeof(M1B));
                }
        ...
        private class M2A : StateMachine
        {
            ...
            private async Task InitOnEntry()
            {
                ...
                var handled = await this.Manager.SendEventAndExecuteAsync(b, new E1());
                this.Assert(!handled);
            }
        }
        ...
        private class M2C : StateMachine
        {
            ...
            private async Task InitOnEntry()
            {
                ...
                var handled = await this.Manager.SendEventAndExecuteAsync(d, new E1());
                this.Assert(handled);
            }
        }
        ...
        private class M3A : StateMachine
        {
            ...
            {
                var e = new E4();
                var m = await this.Manager.CreateActorAndExecuteAsync(typeof(M3B));
                var handled = await this.Manager.SendEventAndExecuteAsync(m, e);
                this.Assert(handled);
                this.Assert(e.X == 1);
            }
        }
        ...
        private class M4A : StateMachine
        {
            ...
            private async Task InitOnEntry()
            {
                var m = await this.Manager.CreateActorAndExecuteAsync(typeof(M4B), new E2(this.Id));
                var handled = await this.Manager.SendEventAndExecuteAsync(m, new E1());
                this.Assert(handled);
            }
        }
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        private class M5A : StateMachine
        {
            ...
            private async Task InitOnEntry()
            {
                var m = await this.Manager.CreateActorAndExecuteAsync(typeof(M5B));
                var handled = await this.Manager.SendEventAndExecuteAsync(m, new E1());
        ...
        private class M6A : StateMachine
        {
            ...
            private async Task InitOnEntry(Event e)
            {
                var m = await this.Manager.CreateActorAndExecuteAsync(typeof(M6B), e);
                var handled = await this.Manager.SendEventAndExecuteAsync(m, new E1());
        ...
        private class M7A : StateMachine
        {
            ...
            private async Task InitOnEntry()
            {
                var m = await this.Manager.CreateActorAndExecuteAsync(typeof(M7B));
                var handled = await this.Manager.SendEventAndExecuteAsync(m, new E1());
                this.Assert(handled);
            }
        }
        ...
        private class M8A : StateMachine
        {
            ...
            private async Task InitOnEntry()
            {
                var m = await this.Manager.CreateActorAndExecuteAsync(typeof(M8B));
                var handled = await this.Manager.SendEventAndExecuteAsync(m, new E1());
                this.Assert(handled);
            }
        }
