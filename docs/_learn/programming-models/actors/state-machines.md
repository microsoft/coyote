...
The nested state classes can be private.
...
Use `[OnEventPushState(...)]` or `RaisePushStateEvent` in code to push a new state:
...
Default events can also invoke goto, and push state transitions, which brings up an interesting
case where you can actually implement an infinite ping pong using the following:
...
The difference between this and a timer based ping-pong is that this will run as fast as the
Coyote runtime can go.  So you have to be careful using `DefaultEvents` like this as it could
...
The client of this state machine can send any event it wants and it will cause a transition
to the `CatchAll` state where it will be handled by the `OnCatchAll` method.  For example:
...
First of all only one action per specific event type can be defined on a given `State`, so the following
would be an error:
...
If multiple states on the stack of active states define an action for
a specific event type then the action closest to the top of the stack takes precedence.
For example:
...
`WildcardEvent` but a pushed state can override a specific event type actions with a `WildcardEvent`.
