...
| name | description |
| --- | --- |
| [CurrentOperation](Actor/CurrentOperation) { get; set; } | An optional operation associated with the current event being handled. An actor that handles an event can choose to complete the operation with a result object. Typically the Operation will be an Operation{T} where the target Actor knows what type the result is. |
