...
- **Raft.Mocking** - demonstrates how to use mocks to systematically test the CloudMessaging sample
  ...
  detail in [Raft actor service (mocked)](raft-mocking) tutorial.
...
3. Create the Coyote `IActorRuntime` using `ActorRuntimeFactory.Create`.
...
![raft_server](/coyote/assets/images/RaftStateMachine.svg)
